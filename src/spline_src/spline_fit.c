...
void sbart(double* penalt, double* dofoff, double* xs, double* ys, double* ws,
           double* ssw, int* n, double* knot, int* nk, double* coef, double* sz,
           double* lev, double* crit, int* icrit, double* spar, int* ispar,
           int* iter, double* lspar, double* uspar, double* tol, double* eps,
           int* isetup, double* xwy, double* hs0, double* hs1, double* hs2,
           double* hs3, double* sg0, double* sg1, double* sg2, double* sg3,
           double* abd, double* p1ip, double* p2ip, int* ld4, int* ldnk,
           int* ier)
{
...
#define BIG_f (1e100)
  ...
  */
  ...
  { /* Value of spar supplied */
    *lspar = ratio* pow(16., *spar * 6. - 2.);
    F77_CALL(sslvrg)(penalt, dofoff, xs, ys, ws, ssw, n, knot, nk, coef, sz,
                     lev, crit, icrit, lspar, xwy, hs0, hs1, hs2, hs3, sg0, sg1,
  ...
  *spar = x;
  *lspar = ratio* pow(16., *spar * 6. - 2.);
  F77_CALL(sslvrg)(penalt, dofoff, xs, ys, ws, ssw, n, knot, nk, coef, sz, lev,
                   crit, icrit, lspar, xwy, hs0, hs1, hs2, hs3, sg0, sg1, sg2,
  ...
  { /* L20: */
    ...
    if (tracing)
    {
      if (*iter == 1)
      { /* write header */
        printf("sbart (ratio = %15.8g) iterations;"
               " initial tol1 = %12.6e :\n"
      ...
      printf("%11.8f %14.9g %9.4e %11.5g", x, CRIT(fx), b - a, e);
      Fparabol = FALSE;
    }
...
void splineFit(double* xs, double* ys, double* knot, int n, double dofoff,
               double* results)
{
  ...
  double eps = 2e-8;
  int isetup = 0;
  double* xwy = (double*)malloc(sizeof(double) * (n + 2) * (n + 2));
  double* hs0 = (double*)malloc(sizeof(double) * (n + 2));
  double* hs1 = (double*)malloc(sizeof(double) * (n + 2));
  double* hs2 = (double*)malloc(sizeof(double) * (n + 2));
  double* hs3 = (double*)malloc(sizeof(double) * (n + 2));
  double* sg0 = (double*)malloc(sizeof(double) * (n + 2));
  double* sg1 = (double*)malloc(sizeof(double) * (n + 2));
  double* sg2 = (double*)malloc(sizeof(double) * (n + 2));
  double* sg3 = (double*)malloc(sizeof(double) * (n + 2));
  double* abd = (double*)malloc(sizeof(double) * 4 * (n + 2));
  double* p1ip = (double*)malloc(sizeof(double) * 4 * (n + 2));
  double* p2ip = (double*)malloc(sizeof(double) * (n + 2));
  int ld4 = 4;
  int ldnk = 1;
  int ier = 1;
  int order = 0;
  sbart(&penalt, &dofoff, xs, ys, ws, &ssw, &n, knot, &nk, coef, sz, lev, &crit,
        &icrit, &spar, &ispar, &iter, &lspar, &uspar, &tol, &eps, &isetup, xwy,
